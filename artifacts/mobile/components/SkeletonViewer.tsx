/**
 * SkeletonViewer — interactive anatomical SVG skeleton.
 * Works on iOS, Android, and web (no native GL modules needed).
 * Drag left/right for 3-D parallax. Tap a bone to select it.
 */
import React, {
  forwardRef,
  useImperativeHandle,
...
function BoneG({
  ...
  return (
    <G
      fill={s ? "url(#sel)" : "url(#bg)"}
      stroke={s ? SEL_S : STROKE}
      strokeWidth={s ? SW * 1.6 : SW}
      onPress={onPress}
    >
      {children}
    </G>
...
>(function SkeletonViewer({ onBoneSelect }, ref) {
  ...
  function B(id: string, info: BoneInfo, ch: React.ReactNode) {
    return (
      <BoneG key={id} id={id} sel={sel} onPress={() => pick(id, info)}>
        {ch}
      </BoneG>
    );
  }
    ...
    <View style={s.root} {...pan.panHandlers}>
      <Animated.View
        style={[
          ...
          { transform: [{ perspective: 900 }, { rotateY: rotDeg }] },
        ]}
      >
        <Svg
          viewBox="0 0 200 438"
        ...
        >
          ...
          <G transform="scale(1, 0.6)">
            ...
            {/* ── Cervical vertebrae ── */}
            {[...Array(7)].map((_, i) =>
              B(`cv${i}`, { name:`C${i+1} Vertebra`, latinName:`Vertebra cervicalis ${i+1}`, region:"vertebral-column", boneId:"atlas" }, <>
            ...
            {/* ── Thoracic vertebrae ── */}
            {[...Array(12)].map((_, i) =>
              B(`tv${i}`, { name:`T${i+1} Vertebra`, latinName:`Vertebra thoracica ${i+1}`, region:"vertebral-column", boneId:"typical-thoracic" }, <>
            ...
            {/* ── Lumbar vertebrae ── */}
            {[...Array(5)].map((_, i) =>
              B(`lv${i}`, { name:`L${i+1} Vertebra`, latinName:`Vertebra lumbalis ${i+1}`, region:"vertebral-column", boneId:"typical-lumbar" }, <>
