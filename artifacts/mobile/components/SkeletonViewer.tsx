/**
 * SkeletonViewer — WEB fallback
 * Metro picks this for web (Expo web / Replit preview).
 * Native (iOS/Android) gets SkeletonViewer.native.tsx which uses WebView + real 3D GLB.
 */
import React, {
  forwardRef,
  useImperativeHandle,
...
function BG({ id, sel, onPress, ch }: { id:string; sel:string|null; onPress:()=>void; ch:React.ReactNode }) {
  const on = sel === id;
  return (
    <G fill={on?"url(#sel)":"url(#bg)"} stroke={on?SEL:STROKE} strokeWidth={on?SW*1.6:SW} onPress={onPress}>
      {ch}
    </G>
  );
}
...
const SkeletonViewer = forwardRef<SkeletonViewerRef, { onBoneSelect: (b: BoneInfo | null) => void }>(
  function SkeletonViewer({ onBoneSelect }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      resetView() { setSel(null); onBoneSelect(null); Animated.spring(rot, { toValue:0, useNativeDriver:false }).start(); },
      setMode() {},
    }));
    ...
    const pan = useRef(PanResponder.create({
      ...
      onPanResponderMove:   (_, g) => rot.setValue(g.dx * 0.4),
      onPanResponderRelease: () => Animated.spring(rot, { toValue:0, useNativeDriver:false, friction:6 }).start(),
    })).current;
    ...
    function B(id: string, info: BoneInfo, ch: React.ReactNode) {
      return <BG key={id} id={id} sel={sel} onPress={() => pick(id, info)} ch={ch} />;
    }

    const deg = rot.interpolate({ inputRange:[-200,200], outputRange:["-28deg","28deg"] });

    return (
      <View style={s.root} {...pan.panHandlers}>
        <Animated.View style={[StyleSheet.absoluteFill, { transform:[{perspective:900},{rotateY:deg}] }]}>
              ...
              {([{w:32,d:5,y:231},{w:40,d:7,y:242},{w:48,d:9,y:253},{w:55,d:11,y:264},{w:60,d:13,y:275},{w:64,d:15,y:286},{w:65,d:17,y:297},{w:64,d:17,y:308},{w:61,d:15,y:319},{w:56,d:13,y:330},{w:49,d:11,y:341},{w:41,d:9,y:352}] as {w:number;d:number;y:number}[]).map((r,i)=>(
                <G key={`rp${i}`}>
                  {B(`ribL${i}`,{name:i===0?"First Rib":"Rib",latinName:`Costa ${i+1}`,region:"thorax",boneId:i===0?"first-rib":"typical-rib"},
                    <Path d={`M91,${r.y} C78,${r.y} ${100-r.w},${r.y+r.d*.5} ${102-r.w},${r.y+r.d} C${108-r.w},${r.y+r.d+2} 87,${r.y+r.d*.8} 91,${r.y+3}`} fill="none" strokeWidth={1.8} strokeLinecap="round" />)}
                  {B(`ribR${i}`,{name:i===0?"First Rib":"Rib",latinName:`Costa ${i+1}`,region:"thorax",boneId:i===0?"first-rib":"typical-rib"},
                    <Path d={`M109,${r.y} C122,${r.y} ${100+r.w},${r.y+r.d*.5} ${98+r.w},${r.y+r.d} C${92+r.w},${r.y+r.d+2} 113,${r.y+r.d*.8} 109,${r.y+3}`} fill="none" strokeWidth={1.8} strokeLinecap="round" />)}
                </G>
              ))}
