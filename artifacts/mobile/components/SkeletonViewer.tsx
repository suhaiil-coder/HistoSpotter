/**
 * SkeletonViewer — platform-aware skeleton renderer.
 *
 * Native (iOS/Android via Expo Go):
 *   → expo-gl + Three.js + real NIH 3D GLB model, PBR bone material,
 *     3-point lighting, touch rotation.
 *
 * Web (Replit preview / Expo web):
 *   → react-native-svg anatomical illustration (radial-gradient bone depth,
 *     BioDigital-style blue selection). WebGL is not available in the preview
 *     iframe, so we show a high-quality SVG fallback.
 */
import React, {
  forwardRef, useEffect, useImperativeHandle, useRef, useState,
} from "react";
import {
  Animated, PanResponder, Platform, StyleSheet, View,
} from "react-native";
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// NATIVE — expo-gl + Three.js + real GLB model
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
let GLView: any = null;
let Asset: any = null;
let THREE: any = null;
...
if (!IS_WEB) {
  ...
  THREE  = require("three");
  GLTFLoader = require("three/examples/jsm/loaders/GLTFLoader.js").GLTFLoader;
}

function getBoneFromNormY(ny: number): BoneInfo {
...
function Viewer3D({
  ...
  const rotRef      = useRef({ theta: 0, phi: 0 });
  ...
  const pan = useRef(PanResponder.create({
    ...
    onPanResponderGrant: (e) => {
      ...
      touchRef.current = { x: e.nativeEvent.locationX, y: e.nativeEvent.locationY, t: Date.now() };
    },
    onPanResponderMove: (_, g) => {
      rotRef.current.theta += g.dx * 0.008;
      rotRef.current.phi   = Math.max(-0.85, Math.min(0.85, rotRef.current.phi + g.dy * 0.006));
    },
    onPanResponderRelease: (e, g) => {
      if (Math.abs(g.dx) < 8 && Math.abs(g.dy) < 8 && Date.now() - touchRef.current.t < 350) {
        const ny = e.nativeEvent.locationY / sizeRef.current.h;
        onBoneSelect(getBoneFromNormY(ny));
      }
    },
  })).current;
  ...
  async function onContextCreate(gl: any) {
    ...
    const renderer = new THREE.WebGLRenderer({ context: gl, alpha: false });
    ...
    // 3-point lighting
    const key  = new THREE.DirectionalLight(0xfff3e0, 1.6); key.position.set(-2, 3, 4); scene.add(key);
    const fill = new THREE.DirectionalLight(0xd0e8ff, 0.5); fill.position.set(3, 1, 2); scene.add(fill);
    const rim  = new THREE.DirectionalLight(0xffe4b0, 0.35); rim.position.set(0, -1, -3); scene.add(rim);
    ...
    try {
      ...
      loader.load(uri, (gltf: any) => {
        ...
        const mat = new THREE.MeshStandardMaterial({ color: 0xEBE0CC, roughness: 0.72, metalness: 0 });
        model.traverse((c: any) => { if (c.isMesh) { c.material = mat; c.castShadow = true; } });
        modelRef.current = model;
        scene.add(model);
      }, undefined, (err: any) => console.error("[3D] GLB load error", err));
    } catch (e) {
      console.error("[3D] asset error", e);
    }

    function animate() {
      rafRef.current = requestAnimationFrame(animate);
      if (modelRef.current) {
        if (autoRef.current) rotRef.current.theta += 0.003;
        modelRef.current.rotation.y = rotRef.current.theta;
        modelRef.current.rotation.x = rotRef.current.phi;
        camera.position.z = 3.2 / Math.max(0.3, Math.min(3, zoomRef.current));
      }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// (only imported when IS_WEB to avoid bundling SVG on native 3D path)
const {
...
function BoneG({ id, sel, onPress, children }: { id: string; sel: string | null; onPress: () => void; children: React.ReactNode }) {
  const s = sel === id;
  return (
    <G fill={s ? "url(#sel)" : "url(#bg)"} stroke={s ? SEL_S : STROKE}
      strokeWidth={s ? SW_ * 1.6 : SW_} onPress={onPress}>
      {children}
    </G>
  );
}
...
function ViewerSVG({
  ...
  const pan2 = useRef(PanResponder.create({
    onStartShouldSetPanResponder: () => true,
  ...
  return (
    <View style={styles.root} {...pan2.panHandlers}>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// Unified export — picks renderer based on platform
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
